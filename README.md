...
- **Query syntax**: AND (space), **OR** (`|`), **NOT** (`!`), **grouping** `( )` (e.g. `report (2023 | 2024) !draft` — nestable; quote a literal paren as `"(x86)"`), `"quoted phrases"`, `*`/`?` wildcards, regex, and `ext:`/`path:`/`file:`/`folder:`/`size:`/`dm:`/`dc:`/`da:`/`attrib:` functions (e.g. `ext:dll | ext:exe`, `report !draft size:>1mb`, `dm:thisweek`, `dc:2024-01-01..2024-06-30`, `attrib:h`). The date functions filter by **m**odified / **c**reated / **a**ccessed time and accept keywords (`today`, `yesterday`, `thisweek`/`thismonth`/`thisyear`, …), `YYYY[-MM[-DD]]` dates, `>`/`>=`/`<`/`<=` comparisons, and `A..B` ranges.
- **Search inside files** with `content:` — e.g. `*.log content:"timeout"` finds log files whose **contents** contain "timeout". It narrows by filename via the index first (so it's fast), then reads only those files' bodies in your own user account (the background service never reads file contents). Text and BOM'd UTF‑16; binary files are skipped.
- **Search builder** (toolbar **Builder**): a friendly form that composes a query from fields — name, **contents**, extensions, size, date (modified/created/accessed), files-vs-folders, attributes, and match options — with a live preview of the query it builds, so the syntax above is discoverable without memorising it.
...
- **Options dropdown** (toolbar): a ticked checklist of match modes — match case, whole word, regular expression, match full path — so you can see at a glance which are on (the button shows the active count).
